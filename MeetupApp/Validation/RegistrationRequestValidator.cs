...
namespace MeetupApp.Validation
{
    public class RegistrationRequestValidator : AbstractValidator<RegistrationRequest>
    {
        public RegistrationRequestValidator()
        {
            ...
            RuleFor(u => u.Login).NotNull().NotEmpty().Length(5, 50).WithMessage(u => $"{nameof(u.Login)} shouldn't be null, empty or with invalid length!");
            RuleFor(u => u.Password).Length(8, 16).WithMessage(u => $"{nameof(u.Password)} should contain from 8 to 16 symbols!");
